...
((Joomla, document) => {
  ...
  Joomla.submitbuttonUpload = () => {
    const form = document.getElementById('uploadForm'); // do field validation
...
((Joomla, document) => {
  ...
  PreUpdateChecker.run = () => {
    ...
    const extensions = document.querySelectorAll(PreUpdateChecker.config.selector); // If there are no extensions to be checked we can exit here
    ...
    [].slice.call(extensions).forEach(extension => {
  ...
  PreUpdateChecker.checkCompatibility = (node, callback) => {
    ...
    Joomla.request({
      ...
      onSuccess(data) {
        const response = JSON.parse(data); // Extract the data from the JResponseJson object
  ...
  PreUpdateChecker.setResultView = extensionData => {
    let html = ''; // const direction = (document.dir !== undefined) ? document.dir : document.getElementsByTagName('html')[0].getAttribute('dir');
