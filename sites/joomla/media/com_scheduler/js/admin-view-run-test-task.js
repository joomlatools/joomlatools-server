...
const initRunner = () => {
  ...
  const modal = document.getElementById('scheduler-test-modal'); // Task output template
      ...
      onSuccess: (data, xhr) => {
        ...
        if (output && output.success && output.data) {
          ...
          if (output.data.output) {
            const result = Joomla.sanitizeHtml(output.data.output, null, sanitiseTaskOutput); // Can use an indication for non-0 exit codes
